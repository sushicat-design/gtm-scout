...
function openProposalModal(r){
    ...
    '<div style="background:var(--sur);border:1px solid var(--bor2);border-radius:var(--r);padding:28px;width:100%;max-width:560px;margin:auto;display:flex;flex-direction:column;gap:0">'+
      ...
      '<div style="display:grid;grid-template-columns:1fr auto;gap:10px;align-items:center;margin-bottom:10px">'+
        '<div style="position:relative">'+
          ...
          '<label id="pm-logo-label" style="position:absolute;right:8px;top:50%;transform:translateY(-50%);background:var(--sur2);border:1px solid var(--bor2);color:var(--tx2);font-size:11px;font-weight:600;padding:4px 10px;border-radius:4px;cursor:pointer;white-space:nowrap">'+
            'Upload'+
            '<input type="file" id="pm-logo-file" accept="image/*" style="display:none">'+
          '</label>'+
        '</div>'+
  ...
  setTimeout(function(){
    ...
    if(lf)lf.onchange=function(){
      ...
      r.onload=function(e){var el=document.getElementById('pm-logo');if(el)el.value=e.target.result;};
      r.readAsDataURL(f);
    };
  },50);
}
